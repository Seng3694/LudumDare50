...
class TransitionState : public gjt::GameState
{
  public:
    ...
    virtual void load() override
    {
        ...
        pressToContinueText.setString(
            "Press R to restart or Escape to go back");
        localBounds = pressToContinueText.getLocalBounds();
        pressToContinueText.setOrigin(
            localBounds.width / 2.0f, localBounds.height / 2.0f);
        pressToContinueText.setPosition(
            game->getWindowWidth() / 2.0f,
            game->getWindowHeight() - localBounds.height - 50.0f);
        ...
        auto bannerTexture = content->loadFromFile<sf::Texture>(
